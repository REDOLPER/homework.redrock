    ...
    switch (this.#PromiseState) {
      ...
      case FULFILLED:
        nextPromise = new MyPromise((resolve, reject) => {
          // 包裹一个setTimeout可以模拟微任务的效果 // 但是setTimeout不是宏任务嘛（？
          setTimeout(() => {
            try {
      ...
      case PENDING:
        nextPromise = new MyPromise((resolve, reject) => {
          ...
          _this.#onRejectedCallbacks.push(() => {
            setTimeout(() => {
              try {
                // 尝试调用 onRejected 函数，并将 _this.PromiseResult 作为参数传递；如果 onRejected 未定义，则直接使用 _this.PromiseResult
                const returnValue = onRejected ? onRejected(_this.PromiseResult) : _this.PromiseResult;
                if (returnValue instanceof MyPromise) {
                  returnValue.then(resolve, reject);
                } else {
